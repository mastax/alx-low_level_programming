 ...
 * Auth: EL MOSTAFA EL BAHTOURI (mastax)
 */
 ...
 *
 * Description: if the file is not an ELD file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int i = 0;
 ...
 * @e_entry: the adrress of ELF entry point.
 * @e_ident: pointer to an array containing the ELF class.
 */
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
 ...
 *
 * Description: If the file cannot be closed - exite code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * the function fails - exite code 98.
 */
